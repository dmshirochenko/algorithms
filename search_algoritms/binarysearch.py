...
def binary_search(checked_list, element_to_check):
    ...
    while (diff_right_left != 1):
        ...
        if (sorted_checked_list[mid_point_index] > element_to_check): #go to check next part on left side
            right = mid_point_index
        elif(sorted_checked_list[mid_point_index] < element_to_check): #go to check next part on right side
...
class SmokeBinarySearchTest(unittest.TestCase):
    ...
    def test_random_compared_with_linear(self):
        ...
        self.assertEqual(binary_search(checked_list, element_to_check), linear_search(checked_list, element_to_check))
...
if __name__ == '__main__':
    ...
    for i in [10_000, 100_000, 1_000_000, 10_000_000]:
        ...
        print('Binary search(with list sorting). Time to find in {0} elemets = {1}'.format(i, time.perf_counter() - timer_start))

        timer_start = time.perf_counter()
        linear_search(checked_list, element_to_check)
        print('Linear Search. Time to find in {0} elemets = {1}\n'.format(i, time.perf_counter() - timer_start))
