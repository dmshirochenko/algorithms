...
def merge_sort(items_to_sort):
    ...
    return merging_lists(merge_sort(left_side), merge_sort(right_side))
...
class SmokeSortTest(unittest.TestCase):
    ...
    def test_negative_numbers_array(self):
        with self.subTest(items_to_sort = [-9, -3, -5, -7, -9]):
            self.assertEqual(merge_sort([-9, -3, -5, -7, -9]), [-9, -9, -7, -5, -3])
        with self.subTest(items_to_sort=[-9, 3, 5, 7, -9]):
            self.assertEqual(merge_sort([-9, 3, 5, 7, -9]), [-9, -9, 3, 5, 7])
    ...
    def test_all_possitive(self):
        with self.subTest(items_to_sort=[4, 6, 7, 1, 3]):
            self.assertEqual(merge_sort([4, 6, 7, 1, 3]), [1, 3, 4, 6, 7])
        with self.subTest(items_to_sort=[0, 6, 7, 1, 0]):
            self.assertEqual(merge_sort([0, 6, 7, 1, 0]), [0, 0, 1, 6, 7])
    ...
    def test_hundred_elemets_againts_default_sort(self):
        ...
        self.assertEqual(merge_sort(items_to_sort.copy()), sorted(items_to_sort))
